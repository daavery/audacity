...
void EffectEqualization::PopulateOrExchange(ShuttleGui & S)
{
   ...
   szr2->Add( mdBMaxSlider, 1, wxALIGN_LEFT|wxALL, 4 ); //remove |wxALIGN_CENTER_VERTICAL
   mdBMinSlider = new wxSlider(parent, ID_dBMin, DEF_dBMin, MIN_dBMin, MAX_dBMin,
      wxDefaultPosition, wxDefaultSize, wxSL_VERTICAL|wxSL_INVERSE);
   szr2->Add( mdBMinSlider, 1, wxALIGN_LEFT|wxALL, 4 );// remove |wxALIGN_CENTER_VERTICAL
   szr1->Add( szr2, 0, wxEXPAND|wxALL, 4 ); // remove  |wxALIGN_CENTRE
   ...
   szr1->Add( mPanel, 1, wxEXPAND); // remove  |wxALIGN_CENTRE
   ...
   szrV->Add( szr1, 1, wxEXPAND|wxALL, 0 );  //remove  |wxALIGN_CENTER
   ...
   for (int i = 0; (i < NUMBER_OF_BANDS) && (kThirdOct[i] <= mHiFreq); ++i)
   {
      mSliders[i] = new wxSlider(parent, ID_Slider + i, 0, -20, +20,
         ...
         wxSL_INVERSE);
	  szrG->Add(mSliders[i], 0, wxEXPAND); // remove | wxALIGN_CENTER
